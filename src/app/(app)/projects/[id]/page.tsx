...
import { apiRequest, listSitesRequest, listUsersRequest } from '@/lib/api';
...
export default function ProjectCommandCenterPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const [projectData, passData, siteData, userData, auditData] = await Promise.all([
        ...
        listUsersRequest(token, { pageSize: 250 }).catch(() => []),
        apiRequest<AuditEntry[]>(`/projects/${params.id}/activity`, { token }).catch(() => []),
      ]);
      setProject(projectData);
      setWorkPasses(passData);
      setSites((siteData as Site[]).filter((site) => projectData.siteIds.includes(site.id)));
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <WorkerAccessDialog open={requestOpen} onOpenChange={setRequestOpen} token={token ?? ''} project={project} sites={sites} currentUser={user} onCreated={load} />
...
function WorkerAccessDialog({ open, onOpenChange, token, project, sites, currentUser, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; token: string; project: ProjectRecord; sites: Site[]; currentUser?: User | null; onCreated: () => Promise<void> }) {
  ...
  const availableWorkers = project.members
    .filter((member) => member.role === 'Worker')
    .filter((member) => !currentUser?.contractorId || member.contractorId === currentUser.contractorId)
