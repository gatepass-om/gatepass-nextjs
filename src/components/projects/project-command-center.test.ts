...
test('work-pass actions follow contractor, consultant, then supervisor approval stages', () => {
  ...
  assert.deepEqual(getWorkPassActions(submitted, project, { id: 'consultant-reviewer-1', role: 'Contractor Admin' }), ['approve', 'reject']);
...
test('access-request work-pass queue exposes consultant and operator decisions', () => {
  ...
  const consultantQueue = getWorkPassQueueItems(
    ...
    [project],
    { id: 'consultant-reviewer-1', role: 'Supervisor' },
  );
  ...
  assert.deepEqual(operatorQueue.map((item) => item.actions), [[], ['second-approve', 'reject']]);
