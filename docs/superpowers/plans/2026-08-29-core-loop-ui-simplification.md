...
- `npm run build` is intermittently affected by the same guard (`next.config.ts` rejects `NEXT_PUBLIC_BACKEND_URL` pointing at a `*.devtunnels.ms` host whenever `NODE_ENV=production`, which the build script always sets). Observed during Task 11: the implementer's build succeeded, but an independent re-run immediately after (identical `.env.local` content, clean `.next` cache) failed with the same guard error — the cause is environmental (likely a transient env value from a concurrent task's live-verification session), not a code defect, and unrelated to whatever files a given task touches. Treat a build failure that cites this exact guard as inconclusive, not a task regression; typecheck remains the reliable signal.
...
3. **A second work-pass status/action duplication was found and confirmed by the final whole-branch review — this is now a real follow-up, not a hypothetical.** `project-work-pass-queue.tsx` (rendered on `/projects`) and `WorkPassRow` in `projects/[id]/page.tsx` still maintain two independent status-label/color maps and two independent action-dispatch implementations for the same `WorkPass` record — the same class of defect Task 6 fixed one copy of ("Pending operator approval" → "Pending supervisor approval"). Confirmed current divergence: the `Approved` status reads "Approved" in one map and "Access granted" in the other; the reject dialog's title/button text also differs between the two. Fix: extract a single `workPassStatusPresentation(status)` (and ideally the reject-dialog copy) into `project-command-center.ts` and have both renderers consume it — a small, well-scoped follow-up, not a full merge of the two renderers.
4. **Three spec targets were not completed by this plan's 12 tasks — carry these forward, they were not deliberate non-goals:**
   - **`/profile/page.tsx` was never touched.** The spec's Phase 1 headline outcome — "the exact same component renders at `/profile` for self-service, replacing the current divergent profile implementation, so there is one implementation instead of two" — was not implemented. `/users/[id]` was consolidated; `/profile` still has its own separate, divergent worker view. This is the largest gap between spec and delivery.
   - **Plain-language status chips and request-origin tagging (spec Phase 2 §2) were not implemented.** `requests-table.tsx` still renders the raw `{request.status}` value as the status badge (no "To approve"/"Returned"-style relabeling), and rows are not tagged by origin ("Direct request" vs. "via Project: `<name>`"). Only the tab labels themselves got the plain-language treatment (Task 8).
   - **`/dashboard`'s page heading is not literally "Dashboard."** It renders "Good morning, {firstName}" instead. The other three in-scope pages (`/users`, `/projects`, `/access-requests`) do correctly match their nav labels.
